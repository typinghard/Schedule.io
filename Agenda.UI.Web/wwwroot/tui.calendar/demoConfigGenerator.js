...
exec(`find ./examples -name '*.html'`, (error, stdout, stderr) => {
  ...
  */
    fs.writeFile('./tui.calendar/examples/demo.json', JSON.stringify(demoConfig), 'utf8', err => {
        console.log('\n * [COMPLETED] Check the "./tui.calendar/examples/demo.json" file.\n');
  });
...
function generateFileData(filename) {
    const depJsFile = filename.indexOf('example00') > -1 ? './tui.calendar/js/app.js' : './tui.calendar/js/default.css';
    const jsFilePaths = ['./tui.calendar/js/data/calendars.js', './tui.calendar/js/data/schedules.js', depJsFile];
    const cssFilePaths = ['./tui.calendar/css/default.css', './tui.calendar/css/icons.css'];
